...
def get_args_parser():
    ...
    parser.add_argument("--input_size", default=896, type=int, help="images input size")
    ...
    parser.add_argument(
        "--data_path",
        default="/home/manjimin/.storage/pathology/slide_cut",
        type=str,
        help="dataset path",
    )
...
def main(args):
    ...
    # define the model
    model = models_bitnet.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)

    # Construct the DDP model
    model = DDP(model, device_ids=["cuda:0", "cuda:1", "cuda:2", "cuda:3"])
